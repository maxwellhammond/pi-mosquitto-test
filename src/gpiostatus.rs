...
use rppal::{gpio::Gpio, system::{DeviceInfo, Model}};
...
pub(crate) fn print_status() -> Result<(), Box<dyn Error>>{
    println!("Hello, Pi!");
    println!("Let's read some GPIO pins!");
    ...
    // blink an LED?
    // let mut led_pin = Gpio::new()?.get(16)?.into_output();
    // let mut count = 0;
